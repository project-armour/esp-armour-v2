
mpremote cp button.py :button.py
mpremote cp command_handler.py :command_handler.py
mpremote cp connection_handler.py :connection_handler.py
mpremote cp display.py :display.py
mpremote cp hardware.py :hardware.py
mpremote cp heart_rate.py :heart_rate.py
mpremote cp main.py :main.py
mpremote cp pin_device.py :pin_device.py
mpremote cp state.py :state.py
mpremote cp utils.py :utils.py
mpremote cp network_manager.py :network_manager.py
mpremote cp config_default.json :config_default.json
mpremote reset
